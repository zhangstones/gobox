...
| gobox 参数 | 对应原命今参数 | 实现一致性 | 功能说明 |
|------------|---------------|------------|----------|
| `gobox cmp FILE1 FILE2` | `cmp FILE1 FILE2` | ✅ 一致 | 比较两个文件，报告首个差异 |
| `gobox cmp -s FILE1 FILE2` | `cmp -s` | ✅ 一致 | 静默模式，仅返回退出码 |
| `gobox cmp -l FILE1 FILE2` | `cmp -l` | ✅ 一致 | 输出所有不同字节位置和值 |
| `gobox cmp -n NUM FILE1 FILE2` | `cmp -n` | ✅ 一致 | 只比较前 NUM 字节 |
| `gobox cmp FILE -` | `cmp FILE -` | ✅ 一致 | 支持从 stdin 读取一侧输入 |
