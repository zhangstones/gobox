...
命令设计原则、能力收敛边界和“不做什么”的规范统一见 `docs/CMD-DESIGN.md`；本文只定义支持矩阵与用户可见语义。

---

本文档是 gobox 命令级用户可见语义的唯一规格源：支持矩阵、兼容性标签、差异边界和 gobox 扩展语义都只在这里定义；`docs/CMD-DESIGN.md` 负责定义命令设计边界，`docs/TEST-DESIGN.md` 只负责定义如何证明这些规格，`docs/TEST-CASES.md` 只负责把规格映射到具体案例。
...
- `docs/CMD-SPECS.md`、`docs/TEST-DESIGN.md`、`docs/TEST-CASES.md` 的一致性标签与验证强度必须互相对齐，不能出现文档宣称“原生一致”但测试仅按 gobox 合同验证的情况
...
| `gobox lsof -iTCP` | `lsof -iTCP` | ⚠️ 部分一致 | 仅列出 TCP 网络连接（按 NODE 列匹配） |
| `gobox lsof -iUDP` | `lsof -iUDP` | ⚠️ 部分一致 | 仅列出 UDP 网络连接（按 NODE 列匹配） |
