...
func tailCmd(args []string) error {
	...
	for i < len(args) {
		...
		case strings.HasPrefix(arg, "--pid="):
			p, err := strconv.Atoi(arg[5:])
			if err != nil || p <= 0 {
				return fmt.Errorf("invalid PID: %s", arg[5:])
			}
			pid = p
		case arg == "-h" || arg == "--help":
			showHelp = true
...
doneFlags:
	...
	// Follow mode
	if follow {
		return tailFollow(files, lines, followByName, retry, quiet, multipleFiles, sleepInterval, pid)
	}
...
func tailFollow(files []string, lines int, followByName, retry, quiet, multipleFiles bool, sleepInterval float64, pid int) error {
